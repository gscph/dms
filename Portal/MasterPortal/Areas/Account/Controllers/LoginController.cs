    ...
    public class LoginController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl, string invitationCode)
        {
                ...
                || (!ViewBag.Settings.LocalLoginByEmail && string.IsNullOrWhiteSpace(model.Username)))
            {
                ...
                AddErrors(new IdentityError() { Description = " Invalid sign-in attempt. Please enter the correct user access credentials." });
            ...
            {
                AddErrors(new IdentityError() { Description = " Your branch has not been configured. Please contact your system administrator to fix this." });
                return View(GetLoginViewModel(model, null, returnUrl, invitationCode));
            }
        ...
        private void deleteUserBranchCookie()
        {
            if (Request.Cookies["Branch"] != null)
            {
                HttpCookie myCookie = new HttpCookie("Branch");
