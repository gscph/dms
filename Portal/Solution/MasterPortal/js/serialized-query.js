...
+function ($, URI) {
  ...
  function submitSerializedQuery(e) {
    ...
    if ($entitygrid.length == 1 && target) {
        ...
        $('form input[id*="dateFromValue"]').each(function () {
            var dateFrom = $('#dateFromValue' + count).val();
            var dateTo = $('#dateToValue' + count).val();
            var field = $('#dateFromValue' + count).attr('data-entityfield');

            if ((dateFrom !== "" || dateTo !== "") && (typeof field !== 'undefined')) {
                metaFilter = metaFilter + ",date=DateFrom=" + dateFrom + "&DateTo=" + dateTo + "&field=" + field;
            }
            count++;
        });
