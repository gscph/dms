...
namespace GSC.Rover.DMS.BusinessLogic.InventoryMovement
{
    public class InventoryMovementHandler
    {
        ...
        public Entity UpdateProductQuantityDirectly(Entity quantityEntity, int onhand, int available, int allocated, int onOrder, int sold, int inTransit, int damaged, int backOrder)
        {
            if (onhand != 0)
            { 
                var onHandConditionList = new List<ConditionExpression>
                {
                    new ConditionExpression("gsc_status", ConditionOperator.LessEqual, 100000001),//Allocated and Available    
                    new ConditionExpression("gsc_productquantityid", ConditionOperator.Equal, quantityEntity.Id)
                };
                quantityEntity["gsc_onhand"] = CountQuantity(quantityEntity, onHandConditionList);
            }

            if (available != 0)
            {
                var availableConditionList = new List<ConditionExpression>
                {
                    new ConditionExpression("gsc_status", ConditionOperator.Equal, 100000000),// Available    
                    new ConditionExpression("gsc_productquantityid", ConditionOperator.Equal, quantityEntity.Id)
                };
                quantityEntity["gsc_available"] = CountQuantity(quantityEntity, availableConditionList);
            }

            if(allocated != 0)
            {
                var allocatedConditionList = new List<ConditionExpression>
                {
                    new ConditionExpression("gsc_status", ConditionOperator.Equal, 100000001),// Allocated    
                    new ConditionExpression("gsc_productquantityid", ConditionOperator.Equal, quantityEntity.Id)
                };
                quantityEntity["gsc_allocated"] = CountQuantity(quantityEntity, allocatedConditionList);
            }
            ...
            if (sold != 0)
            {
                var soldConditionList = new List<ConditionExpression>
                {
                    new ConditionExpression("gsc_status", ConditionOperator.Equal, 100000002),// Sold
                    new ConditionExpression("gsc_productquantityid", ConditionOperator.Equal, quantityEntity.Id)
                };
                quantityEntity["gsc_sold"] = CountQuantity(quantityEntity, soldConditionList);
            }
        ...
        private int CountQuantity(Entity quantityEntity, List<ConditionExpression> countCondition)
        {
            ...
            {
                return inventoryCollection.Entities.Count;
            }
