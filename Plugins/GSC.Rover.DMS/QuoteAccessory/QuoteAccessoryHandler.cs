...
namespace GSC.Rover.DMS.BusinessLogic.QuoteAccessory
{
    public class QuoteAccessoryHandler
    {
        ...
        private bool HasExistingAccessory(Entity quoteAccessory)
        {
            _tracingService.Trace("Started HasExistingAccessory Method...");
            var quoteId = quoteAccessory.Contains("gsc_quoteid") ? quoteAccessory.GetAttributeValue<EntityReference>("gsc_quoteid").Id
                : Guid.Empty;
            var productId = quoteAccessory.Contains("gsc_productid") ? quoteAccessory.GetAttributeValue<EntityReference>("gsc_productid").Id
                : Guid.Empty;

            EntityCollection quoteAccessoryCollection = CommonHandler.RetrieveRecordsByOneValue("gsc_sls_quoteaccessory", "gsc_quoteid", quoteId, _organizationService,
                null, OrderType.Ascending, new[] { "gsc_productid" });

            if (quoteAccessoryCollection != null && quoteAccessoryCollection.Entities.Count > 0)
            {
                _tracingService.Trace("Quote Accessories retrieved: " + quoteAccessoryCollection.Entities);
                foreach (Entity existingQuoteAccessory in quoteAccessoryCollection.Entities)
                {
                    var compareProductId = existingQuoteAccessory.Contains("gsc_productid") ? existingQuoteAccessory.GetAttributeValue<EntityReference>("gsc_productid").Id
                        : Guid.Empty;
