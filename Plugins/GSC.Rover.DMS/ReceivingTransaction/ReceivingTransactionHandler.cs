...
namespace GSC.Rover.DMS.BusinessLogic.ReceivingTransaction
{
    public class ReceivingTransactionHandler
    {
        ...
        public Entity CancelReceivingTransaction(Entity receivingTransactionEntity)
        {
            ...
            if (receivingTransactionDetailRecords != null && receivingTransactionDetailRecords.Entities.Count > 0)
            {
                ...
                EntityCollection inventoryRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_iv_inventory", "gsc_iv_inventoryid", inventoryId, _organizationService, null, OrderType.Ascending,
                    new[] { "gsc_productquantityid", "gsc_status" });
                ...
                if (inventoryRecords != null && inventoryRecords.Entities.Count > 0)
                {
                    ...
                    EntityCollection productQuantityRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_iv_productquantity", "gsc_iv_productquantityid", productQuantityId, _organizationService, null, OrderType.Ascending,
                        new[] { "gsc_onhand", "gsc_available", "gsc_allocated" });
        ...
        public Entity ReplicatePurchaseOrderDetailFields(Entity receivingTransactionEntity)
        {
            ...
            if (purchaseOrderDetailRecords != null && purchaseOrderDetailRecords.Entities.Count > 0)
            {
                foreach (Entity purchaseOrderDetail in purchaseOrderDetailRecords.Entities)
                {
                    ...
                    receivingTransactionEntity["gsc_productid"] = purchaseOrderDetail.GetAttributeValue<EntityReference>("gsc_productid") != null
                        ? purchaseOrderDetail.GetAttributeValue<EntityReference>("gsc_productid")
                        : null;
                    receivingTransactionEntity["gsc_modelcode"] = purchaseOrderDetail.Contains("gsc_modelcode")
                        ? purchaseOrderDetail.GetAttributeValue<String>("gsc_modelcode")
                        : String.Empty;
                    receivingTransactionEntity["gsc_optioncode"] = purchaseOrderDetail.Contains("gsc_optioncode")
                        ? purchaseOrderDetail.GetAttributeValue<String>("gsc_optioncode")
                        : String.Empty;
        ...
        public Entity InventoryMovementUponReceiving(Entity vehicleReceiving)
        {
            ...
            if (receivingTransactionDetailRecords != null && receivingTransactionDetailRecords.Entities.Count > 0)
            {
                ...
                foreach (Entity vehicleReceivingItem in receivingTransactionDetailRecords.Entities)
                {
                    ...
                    EntityCollection inventoryRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_iv_inventory", "gsc_iv_inventoryid", inventoryId, _organizationService, null, OrderType.Ascending,
                        new[] { "gsc_productquantityid", "gsc_mmpcinvoicedate", "gsc_status", "gsc_productid", "gsc_modelcode", "gsc_optioncode", "gsc_modelyear",
                        "gsc_siteid", "gsc_vin", "gsc_csno", "gsc_productionno", "gsc_engineno"});
                    ...
                    if (inventoryRecords != null && inventoryRecords.Entities.Count > 0)
                    {
                        ...
                        Guid intransitSite = vehicleReceiving.GetAttributeValue<EntityReference>("gsc_intransitsiteid") != null
                                        ? vehicleReceiving.GetAttributeValue<EntityReference>("gsc_intransitsiteid").Id 
                                        : Guid.Empty;
                        Guid destinationSite = vehicleReceiving.GetAttributeValue<EntityReference>("gsc_siteid") != null
                            ? vehicleReceiving.GetAttributeValue<EntityReference>("gsc_siteid").Id 
                            : Guid.Empty;

                        //Get Product Quantity based on Destination Site
                        ...
                        UpdateInventorySite(vehicleReceiving, inventoryEntity, productQuantityId);

                        //Decrease in Onhand and Available in In-Transit Site
                        DecreaseinInTransit(vehicleReceiving, inventoryEntity, quantityId, transactionDate, destinationSite, intransitSite);

                        //Increase in Onhand and Available in Desination Site
                        IncreaseinDestination(vehicleReceiving, inventoryEntity, quantityId, productQuantityId, transactionDate, destinationSite, intransitSite);
        ...
        private void DecreaseinInTransit(Entity vehicleReceiving, Entity inventoryEntity, Guid productQuantityid, DateTime transactionDate, Guid destinationSite, Guid intransitSite)
        {
            InventoryMovementHandler handler = new InventoryMovementHandler(_organizationService, _tracingService);

            EntityCollection quantityRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_iv_productquantity", "gsc_iv_productquantityid", productQuantityid, _organizationService, null, OrderType.Ascending,
           new[] { "gsc_onhand", "gsc_available", "gsc_allocated", "gsc_onorder", "gsc_sold", "gsc_intransit", "gsc_damaged", "gsc_backorder", "gsc_vehiclecolorid", "gsc_vehiclemodelid" });
            ...
            if (quantityRecords != null && quantityRecords.Entities.Count > 0)
            {
                ...
                var inventoryStatus = inventoryEntity.Contains("gsc_status")
                    ? inventoryEntity.GetAttributeValue<OptionSetValue>("gsc_status").Value
                    : 0;

                if (inventoryStatus == 100000000)
                    handler.UpdateProductQuantityDirectly(quantityEntity, -1, -1, 0, 0, 0, 0, 0, 0);
                else if (inventoryStatus == 100000001)
                    handler.UpdateProductQuantityDirectly(quantityEntity, -1, 0, -1, 0, 0, 0, 0, 0);
            }
        }

        private void UpdateInventorySite(Entity vehicleReceiving, Entity inventoryEntity, Guid productQuantityId)
        {
            inventoryEntity["gsc_productquantityid"] = new EntityReference("gsc_iv_productquantity", productQuantityId);
            inventoryEntity["gsc_mmpcinvoicedate"] = vehicleReceiving.GetAttributeValue<DateTime>("gsc_actualreceiptdate");
            _organizationService.Update(inventoryEntity);
        }

        private void IncreaseinDestination(Entity vehicleReceiving, Entity inventoryEntity, Guid oldProductQuantityId, Guid productQuantityId, DateTime transactionDate, Guid destinationSite, Guid intransitSite)
        {
            InventoryMovementHandler handler = new InventoryMovementHandler(_organizationService, _tracingService);

            EntityCollection productQuantityRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_iv_productquantity", "gsc_iv_productquantityid", productQuantityId, _organizationService, null, OrderType.Ascending,
            new[] { "gsc_onhand", "gsc_available", "gsc_allocated", "gsc_onorder", "gsc_sold", "gsc_intransit", "gsc_damaged", "gsc_backorder",
                "gsc_vehiclemodelid", "gsc_vehiclecolorid" });
            ...
            if (productQuantityRecords != null && productQuantityRecords.Entities.Count > 0)
            {
                ...
                {
                    handler.UpdateProductQuantityDirectly(productQuantityRecords.Entities[0], 1, 0, 1, 0, 0, 0, 0, 0);
                    RetrieveInventoryHistory(oldProductQuantityId, productQuantity.Id, inventoryEntity.Id);
                }
            }
        }

        private void RetrieveInventoryHistory(Guid oldProductQuantityId, Guid newProductQuantityId, Guid inventoryId)
        {
             var productQuantityConditionList = new List<ConditionExpression>
                {
                    new ConditionExpression("gsc_productquantityid", ConditionOperator.Equal, oldProductQuantityId),
                    new ConditionExpression("gsc_latest", ConditionOperator.Equal, true),
                    new ConditionExpression("gsc_inventoryid", ConditionOperator.Equal, inventoryId),
                    //new ConditionExpression("gsc_isonorder", ConditionOperator.Equal, false)
                };

             EntityCollection inventoryHistoryCollection = CommonHandler.RetrieveRecordsByConditions("gsc_iv_inventoryhistory", productQuantityConditionList, _organizationService, null, OrderType.Ascending,
                new[] { "gsc_productquantityid"});

            if (inventoryHistoryCollection != null && inventoryHistoryCollection.Entities.Count > 0)
            {
                Entity inventoryHistory = inventoryHistoryCollection.Entities[0];
                inventoryHistory["gsc_productquantityid"] = new EntityReference("gsc_iv_productquantity", newProductQuantityId);
                _organizationService.Update(inventoryHistory);
            }
        }
        ...
        private Guid GetProductQuantity(Entity vehicleReceiving, Entity receivingItem)
        {
            ...
            else
            {
                ...
                String productName = receivingItem.GetAttributeValue<EntityReference>("gsc_productid") != null
        ...
        public Entity UpdateInventoryWBNo(Entity vehicleReceiving)
        {
            EntityCollection receivingTransactionDetailRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_cmn_receivingtransactiondetail", "gsc_receivingtransactionid", vehicleReceiving.Id, _organizationService, null, OrderType.Ascending,
                new[] { "gsc_inventoryid" });
            ...
            if (receivingTransactionDetailRecords != null && receivingTransactionDetailRecords.Entities.Count > 0)
            {
                ...
                new[] { "gsc_warrantybookletno" });

                if (inventoryRecords != null && inventoryRecords.Entities.Count > 0)
                {
                    Entity inventoryEntity = inventoryRecords.Entities[0];
                    inventoryEntity["gsc_warrantybookletno"] = vehicleReceiving.Contains("gsc_warrantybookletno")
                        ? vehicleReceiving.GetAttributeValue<String>("gsc_warrantybookletno")
                        : String.Empty;
                    _organizationService.Update(inventoryEntity);
                }
       ...
       //Check if WB No provided is not used yet.
        public Boolean IsWBNoUnique(Entity vehicleReceiving)
        {
            ...
            if (WBNo != String.Empty && WBNo != null)
            {
                EntityCollection inventoryRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_iv_inventory", "gsc_warrantybookletno", WBNo, _organizationService, null, OrderType.Ascending,
                    new[] { "gsc_warrantybookletno" });
