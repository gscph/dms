    ...
    public class QuoteHandler
    {
        ...
        public Entity ConcatenateVehicleDescription(Entity quoteEntity, string message)
        {
            ...
            if (quoteEntity.Contains("gsc_productid"))
            {
                ...
                EntityCollection productRecords = CommonHandler.RetrieveRecordsByOneValue("product", "productid", productId, _organizationService, null, OrderType.Ascending,
                    new[] { "gsc_vehicletypeid", "gsc_bodytypeid", "gsc_transmission", "gsc_grossvehicleweight", "gsc_vehiclemodelid", "gsc_pistondisplacement", "gsc_fueltype", "gsc_status", "gsc_modelyear",
        ...
        * This formula applies when there is discount applied for Amount Financed.*/
        ...
        public Entity PopulateColorPrice(Entity quoteEntity, String message)
        {
            ...
            //Financing
            if (paymentmode == 100000001 || (paymentmode == 100000002 && CheckifQuoteHasDownPayment(quoteEntity) == true))
            {
        ...
        public void SetLessDiscountValues(Entity quoteEntity)
        {
            ...
            if (paymentmode == 100000001 || paymentmode == 100000002)
            {
                ...
                quoteEntity["gsc_downpaymentamount"] = new Money(downPaymentAmount);
                quoteEntity["gsc_downpaymentamountdisplay"] = new Money(downPaymentAmount);
        ...
        */
        public Entity UpdateGovernmentTax(Entity quoteEntity, String message)
        {
            ...
            if (productRecords != null && productRecords.Entities.Count > 0)
            {
                ...
                var sellPrice = ComputeUnitPrice(productEntity, quoteEntity);

                quoteEntity["gsc_vehicleunitprice"] = new Money(sellPrice);
                quoteEntity["gsc_unitprice"] = quoteEntity["gsc_vehicleunitprice"];
        ...
        private Entity PopulatePaymentSummary(Entity quoteEntity)
        {
            ...
            quoteEntity["gsc_downpaymentdisplay"] = new Money(0);
            quoteEntity["gsc_totaldownpaymentdiscountdisplay"] = new Money(0);
                ...
                if (quoteRecords != null && quoteRecords.Entities.Count > 0)
                {
                    foreach (Entity quote in quoteRecords.Entities)
                    {
                        if (quote.Id != quoteEntity.Id && closeOpportunity == true)
                        {
                            ...
                            {
                                throw new InvalidPluginExecutionException("There are won quotes associated with the opportunity. The opportunity must be manually updated from draft to won.");
                            }
                        }
                    }
        ...
        {
            _tracingService.Trace("Started ConvertPotentialtoCustomer Method...");
            var customer = quoteEntity.Contains("customerid")
                    ? quoteEntity.GetAttributeValue<EntityReference>("customerid") != null
                    ? quoteEntity.GetAttributeValue<EntityReference>("customerid")
                    : null
            ...
            _tracingService.Trace("Ended ConvertPotentialtoCustomer Method...");
        }
        ...
        //Created By : Leslie Baliguat, Created On : 5/2/2017
        //Purpose : When payment mode = Bank PO, check if there is DP to compute or not the amount financed
        public Boolean CheckifQuoteHasDownPayment(Entity quoteEntity)
        {
            if (quoteEntity.Contains("gsc_downpaymentamount"))
            {
                if (quoteEntity.GetAttributeValue<Money>("gsc_downpaymentamount").Value != 0)
                    return true;
            }
            return false;
        }
