...
namespace GSC.Rover.DMS.BusinessLogic.VehiclePurchaseOrder
{
    ...
    */
    public class VehiclePurchaseOrderHandler
    {
        private readonly IOrganizationService _organizationService;
        private readonly ITracingService _tracingService;

        public VehiclePurchaseOrderHandler(IOrganizationService service, ITracingService trace)
        {
        ...
        //Modified By : Jerome Anthony Gerero, Modified On : 5/9/2017
        /*Purpose: Create approval record on approve button click
         * Registration Details: 
         * Event/Message:
         *      Pre/Create:
         *      Post/Update: Approver Level
         *      Post/Create:
         * Primary Entity: Purchase Order
         */
        public Entity CreateApprovalRecord(Entity purchaseOrderEntity)
        {
            ...
            String approverGuid = purchaseOrderEntity.Contains("gsc_approverguid")
                ? purchaseOrderEntity.GetAttributeValue<String>("gsc_approverguid")
                : String.Empty;

            Int32 approverLevel = purchaseOrderEntity.GetAttributeValue<OptionSetValue>("gsc_approverlevel") != null
                ? purchaseOrderEntity.GetAttributeValue<OptionSetValue>("gsc_approverlevel").Value
                : 0;

            String approverLevelName = purchaseOrderEntity.GetAttributeValue<OptionSetValue>("gsc_approverlevel") != null
                ? purchaseOrderEntity.FormattedValues["gsc_approverlevel"]
                : String.Empty;

            String approverName = purchaseOrderEntity.Contains("gsc_approvername")
                ? purchaseOrderEntity.GetAttributeValue<String>("gsc_approvername")
                : String.Empty;

            Int32 approvalStatus = purchaseOrderEntity.GetAttributeValue<OptionSetValue>("gsc_approvalstatus") != null
                ? purchaseOrderEntity.GetAttributeValue<OptionSetValue>("gsc_approvalstatus").Value
                : 0;

            if (String.IsNullOrEmpty(approverGuid) || String.IsNullOrEmpty(approverName)) { return null; }

            Entity purchaseOrderApproval = new Entity("gsc_cmn_purchaseorderapproval");
            purchaseOrderApproval["gsc_purchaseorderid"] = new EntityReference(purchaseOrderEntity.LogicalName, purchaseOrderEntity.Id);
            purchaseOrderApproval["gsc_purchaseorderapprovalpn"] = approverName;
            ...
            //Revert approval status to pending until all available approvers have approved the vpo
            EntityCollection approverDetailRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_cmn_approver", "gsc_approversetupid", new Guid(approverGuid), _organizationService, "gsc_approverlevel", OrderType.Descending,
                new[] { "gsc_cmn_approverid", "gsc_approverlevel" });

            if (approverDetailRecords != null && approverDetailRecords.Entities.Count > 0)
            {
                Entity approverDetail = approverDetailRecords.Entities[0];

                Int32 approverDetailLevel = approverDetail.GetAttributeValue<OptionSetValue>("gsc_approverlevel") != null
                    ? approverDetail.GetAttributeValue<OptionSetValue>("gsc_approverlevel").Value
                    : 0;

                if (approverDetailLevel > approverLevel)
                {
                    purchaseOrderEntity["gsc_approvalstatus"] = new OptionSetValue(100000003);
        ...
        private bool ValidateSubmit(Entity vehiclePurchaseOrder)
        {
            ...
            {
                _tracingService.Trace("Approver setup found...");
