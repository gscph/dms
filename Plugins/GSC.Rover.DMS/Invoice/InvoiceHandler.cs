    ...
    public class InvoiceHandler
    {
        ...
        public Entity ReplicateOrderInfo(Entity invoiceEntity)
        {
            ...
            EntityCollection salesOrderRecords = CommonHandler.RetrieveRecordsByOneValue("salesorder", "salesorderid", salesOrderId, _organizationService, null, OrderType.Ascending,
                 new[] {  "gsc_dealerid", "gsc_branchid", "gsc_recordownerid",
                          ...
                          "gsc_downpaymentamount", "gsc_downpaymentpercentage", "gsc_downpaymentdiscount", "gsc_netdownpayment", "gsc_amountfinanced", "gsc_discountamountfinanced", 
                           ...
                           "gsc_documentstatus", "gsc_downpaymentdisplay", "gsc_totaldownpaymentdiscountdisplay",
                ...
                "gsc_ordercancelleddate", "gsc_createddate", "gsc_invoicedate", "gsc_printeddrdate", "gsc_releaseddate", "gsc_invoicecancelleddate", "gsc_markup" });
            ...
            if (salesOrderRecords != null && salesOrderRecords.Entities.Count > 0)
            {
                ...
                invoiceEntity["gsc_downpaymentdisplay"] = salesOrder.GetAttributeValue<Money>("gsc_downpaymentdisplay") != null
                  ? new Money(salesOrder.GetAttributeValue<Money>("gsc_downpaymentdisplay").Value)
                  : new Money(Decimal.Zero);

                invoiceEntity["gsc_totaldownpaymentdiscountdisplay"] = salesOrder.GetAttributeValue<Money>("gsc_totaldownpaymentdiscountdisplay") != null
                  ? new Money(salesOrder.GetAttributeValue<Money>("gsc_totaldownpaymentdiscountdisplay").Value)
                  : new Money(Decimal.Zero);
        ...
        public Entity ReplicateAllocatedVehicle(Entity invoiceEntity)
        {
            ...
            EntityCollection allocatedVehicleRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_iv_allocatedvehicle", "gsc_orderid", salesOrderId, _organizationService, null, OrderType.Ascending,
                new[] { "gsc_allocatedvehiclepn", "gsc_inventoryid", "gsc_modelcode", "gsc_optioncode",
                    "gsc_productionno", "gsc_csno", "gsc_engineno", "gsc_vin", "gsc_color", "gsc_vehicleallocateddate", "gsc_warrantybookletno" });
            ...
            if (allocatedVehicleRecords != null && allocatedVehicleRecords.Entities.Count > 0)
            {
                ...
                var warrantBookletNumber = allocatedVehicle.GetAttributeValue<String>("gsc_warrantybookletno") != null
                    ? allocatedVehicle.GetAttributeValue<String>("gsc_warrantybookletno")
                    : null;
                ...
                invoicedVehicle["gsc_warrantybookletno"] = warrantBookletNumber;
        ...
        //Created By: Leslie G. Baliguat, Created On: 5/09/17
        /*Purpose: Cannot post invoice when invoiced vehicle is in transit or on-hold
         * Registration Details: 
         * Event/Message:
         *      Post/Update: Sales Invoice Status = gsc_salesinvoicestatus
         * Primary Entity: Invoice
         */
        private Boolean IsVehicleSiteValid(Entity salesInvoice)
        {
            _tracingService.Trace("Started IsVehicleSiteValid method..");

            EntityCollection vehicleRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_iv_invoicedvehicle", "gsc_invoiceid", salesInvoice.Id, _organizationService, null, OrderType.Ascending,
                new[] { "gsc_inventoryid" });

            if (vehicleRecords != null && vehicleRecords.Entities.Count > 0)
            {
                _tracingService.Trace("Retrieve vehicle.");

                Entity vehicle = vehicleRecords.Entities[0];
                Guid inventoryId = CommonHandler.GetEntityReferenceIdSafe(vehicle, "gsc_inventoryid");
               ...
               new[] { "gsc_productquantityid" });
                ...
                if (inventoryRecords != null && inventoryRecords.Entities.Count > 0)
                {
                    ...
                    var pqId = CommonHandler.GetEntityReferenceIdSafe(inventory, "gsc_productquantityid");

                    EntityCollection pqRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_iv_productquantity", "gsc_iv_productquantityid", pqId, _organizationService, null, OrderType.Ascending,
                   new[] { "gsc_siteid" });

                    if (pqRecords != null && pqRecords.Entities.Count > 0)
                    {
                        _tracingService.Trace("Retrieve product quantity.");

                        Entity pqEntity = pqRecords.Entities[0];
                        var siteId = CommonHandler.GetEntityReferenceIdSafe(pqEntity, "gsc_siteid");

                        EntityCollection siteRecords = CommonHandler.RetrieveRecordsByOneValue("gsc_iv_site", "gsc_iv_siteid", siteId, _organizationService, null, OrderType.Ascending,
                       new[] { "gsc_sitetype" });

                        if (siteRecords != null && siteRecords.Entities.Count > 0)
                        {
                            _tracingService.Trace("Retrieve site.");

                            Entity site = siteRecords.Entities[0];
                            var siteType = site.Contains("gsc_sitetype")
                                        ? site.GetAttributeValue<OptionSetValue>("gsc_sitetype").Value
                                        : 0; 

                            _tracingService.Trace("Site Type: " + siteType);

                            if (siteType == 100000002)
                                throw new InvalidProgramException("Cannot release invoiced vehicle that is in transit site.");
                            else if (siteType == 100000003)
                                throw new InvalidProgramException("Cannot release invoiced vehicle that is in on-hold site.");
                        }
                    }
                }
            }
        ...
        public Entity PostInvoice(Entity invoiceEntity)
        {
            ...
            //validation if invoice can be released
            IsVehicleSiteValid(invoiceEntity);
            ...
            if (invoicedVehicleRecords != null && invoicedVehicleRecords.Entities.Count > 0)
            {
                ...
                if (inventoryRecords != null && inventoryRecords.Entities.Count > 0)
                {
                    ...
                    if(productQuantityRecords != null && productQuantityRecords.Entities.Count > 0)
                    {
                        ...
                        _tracingService.Trace("Calling CreateInventoryQuantityAllocated Method");
                       /* inventoryMovementHandler.CreateInventoryQuantityAllocated(invoiceEntity, inventory, productQuantity, invoiceEntity.GetAttributeValue<string>("name"),
                            DateTime.UtcNow, "Sold", Guid.Empty, 100000002);*/
        ...
        public Entity CheckInvoiceIdIfDuplicate(Entity invoiceEntity)
        {
            ...
            if (invoiceRecords != null || invoiceRecords.Entities.Count > 0)
            {
                ...
                if (invoiceNameRecords != null && invoiceNameRecords.Entities.Count > 0)
                {
                    ...
                    {
                        throw new InvalidPluginExecutionException("Sales Invoice ID already exists.");
                    }
