    ...
    public class SalesOrderHandler
    {
        ...
        public Entity ReplicateVehicleDetails(Entity salesOrderEntity, String message)
        {
            ...
            EntityCollection productRecords = CommonHandler.RetrieveRecordsByOneValue("product", "productid", productId, _organizationService, null, OrderType.Ascending,
                new[] { "gsc_enginetype", "gsc_vehicletypeid", "gsc_bodytypeid","gsc_transmission", "gsc_grossvehicleweight", "gsc_pistondisplacement", "gsc_fueltype", "gsc_status", "gsc_sellprice", "gsc_modelyear",
            ...
            if (productRecords != null && productRecords.Entities.Count > 0)
            {
                ...
                salesOrderEntity["gsc_vehicledetails"] = ConcatenateVehicleDescription(product); ;
                ...
                {
                    UpdateVehicleDetails(salesOrderEntity);
                }
            }
        ...
        private String ConcatenateVehicleDescription(Entity product)
        {
            var modelYear = product.Contains("gsc_modelyear")
            ...
            var engineType = product.Contains("gsc_enginetype")
                ...
                : String.Empty;
            var vehicleType = product.GetAttributeValue<EntityReference>("gsc_vehicletypeid") != null
                ? product.GetAttributeValue<EntityReference>("gsc_vehicletypeid").Name
                : string.Empty;
            var bodyType = product.GetAttributeValue<EntityReference>("gsc_bodytypeid") != null
            ...
            String description = "Model Year: " + modelYear +
                    ...
                    "\nFuel Type: " + fuelType +
                    "\nWarranty Years: " + warrantyExpiryDays +
                    "\nWarranty Mileage: " + warrantyMileage +
                    "\nTransmission: " + transmission +
                    "\nOthers: " + otherVehicleDetails;

            return description = description.Remove(description.Length - 2, 2);
        }

        private Entity UpdateVehicleDetails(Entity salesOrderEntity)
        {
            ...
            orderToUpdate["gsc_vehicledetails"] = salesOrderEntity["gsc_vehicledetails"];
            ...
            orderToUpdate["gsc_vehiclebasemodelid"] = salesOrderEntity["gsc_vehiclebasemodelid"];
            _organizationService.Update(orderToUpdate);
        ...
        public Entity SetLessDiscountValues(Entity salesOrderEntity, String message)
        {
            ...
            if (paymentmode == 100000001 || paymentmode == 100000002)
            {
                ...
                if (paymentmode == 100000001 || (paymentmode == 100000002 && CheckifOrderHasDownPayment(salesOrderEntity) == true))
                {
        ...
        public Entity SetVehicleColorAmount(Entity salesOrderEntity, String message)
        {
            ...
            if (paymentmode == 100000001 || (paymentmode == 100000002 && CheckifOrderHasDownPayment(salesOrderEntity) == true))
            {
                ...
                salesOrderEntity["gsc_amountfinanced"] = null;
        ...
        * This formula applies when there is discount applied for Amount Financed.*/
        ...
        public Entity SetTotalCashOutlayAmount(Entity salesOrderEntity, String message)
        {
            ...
            _tracingService.Trace("Ended SetTotalCashOutlayAmount method..");
            return salesOrderEntity;
        }
        ...
        public Entity CreateRequirementChecklist(Entity orderEntity, string message)
        {
            ...
            if (bankId != Guid.Empty && paymentMode == 100000001)
            {
                ...
                var customerType = orderEntity.Contains("gsc_customertype")
                ...
                /*       var DocumentCondition = new List<ConditionExpression>
                           {
                               ...
                               new ConditionExpression("gsc_customertype", ConditionOperator.Equal, bankid)
                           };
                ...
                FilterExpression filterDocument = new FilterExpression();
                filterDocument.FilterOperator = LogicalOperator.Or;
                filterDocument.Conditions.Add(new ConditionExpression("gsc_customertype", ConditionOperator.Equal, customerType));
                if (customerType == 100000000 || customerType == 100000001)
                    filterDocument.Conditions.Add(new ConditionExpression("gsc_customertype", ConditionOperator.Equal, "100000003"));

                QueryExpression retrieveDocument = new QueryExpression("gsc_sls_documentchecklist");
                retrieveDocument.ColumnSet.AddColumns("gsc_documentid", "gsc_documentchecklistpn", "gsc_customertype", "gsc_mandatory", "gsc_documenttype");
                retrieveDocument.Criteria.Conditions.Add(new ConditionExpression("gsc_bankid", ConditionOperator.Equal, bankid));
                retrieveDocument.Criteria.AddFilter(filterDocument);

                EntityCollection Document = _organizationService.RetrieveMultiple(retrieveDocument);
            ...
            {
                //Create default internal documents
                if (message == "Create")
                    CreateDefaultDocumentChecklist(orderEntity, true);
            }
        ...
        //Modified By: Leslie Baliguat, Modified On: 5/18/17
        /* Purpose: Made method reusable for creating default financing documents.
         * isInternal parameter determines if document to be created is the default internal or default financing.
         */
        private void CreateDefaultDocumentChecklist(Entity orderEntity, Boolean isInternal)
        {
            var branchId = CommonHandler.GetEntityReferenceIdSafe(orderEntity, "gsc_branchid");

            var internalDocumentCondition = new List<ConditionExpression>
            {
                new ConditionExpression("gsc_documenttype", ConditionOperator.Equal, isInternal),
       ...
       */
        ...
        private decimal ComputeUnitPrice(Entity productEntity, Entity salesOrderEntity)
        {
            ...
            {
                throw new InvalidPluginExecutionException("There is no effective Price List for the selected Vehicle.");
            }
        ...
        //Created By: Leslie Baliguat, Created On: 5/16/2017
        /*Purpose: Update Vehicle Unit Price when Markup % chanages
        * Registration Details:
        * Event/Message: 
        *      Post/Update: gsc_markup
        * Primary Entity: SalesOrder
        */
        public Entity UpdateGovernmentTax(Entity salesOrderEntity, String message)
        {
            ...
            if (productRecords != null && productRecords.Entities.Count > 0)
            {
                Entity productEntity = productRecords.Entities[0];

                var sellPrice = ComputeUnitPrice(productEntity, salesOrderEntity);

                salesOrderEntity["gsc_vehicleunitprice"] = new Money(sellPrice);
                salesOrderEntity["gsc_unitprice"] = salesOrderEntity["gsc_vehicleunitprice"];
                ...
                if (message == "Update")
                {
                    _tracingService.Trace("Update Order.");

                    Entity orderToUpdate = _organizationService.Retrieve(salesOrderEntity.LogicalName, salesOrderEntity.Id,
                        new ColumnSet("gsc_downpaymentamount", "gsc_netprice", "gsc_totalcashoutlay", "gsc_amountfinanced", "gsc_totalamountfinanced",
                        "gsc_vatablesales", "gsc_vatexemptsales", "gsc_vatexemptsales", "gsc_zeroratedsales", "gsc_totalsales", "gsc_vatamount",
                        "gsc_totalamountdue", "gsc_unitprice", "gsc_vehicleunitprice"));

                    _tracingService.Trace("Retrieve Order Details.");
                    ...
                    _organizationService.Update(orderToUpdate);
                    _tracingService.Trace("Order Updated");

                    return orderToUpdate;
                }
            }
            _tracingService.Trace("Ending UpdateGovernmentTax Method...");
            return salesOrderEntity;
        }
        ...
        public bool ValidateDelete(Entity salesOrderEntity)
        {
            ...
            var salesOrderStatus = salesOrderEntity.Contains("gsc_statuscopy")
                ? salesOrderEntity.GetAttributeValue<OptionSetValue>("gsc_statuscopy").Value
                : 0;
            if (salesOrderStatus == 100000000)
            {
                if (salesOrderEntity.Contains("quoteid"))
                    return false;
                else return true;
            }
            else
        ...
        private Entity PopulatePaymentSummary(Entity salesOrderEntity)
        {
            ...
            if (quoteRecords != null && quoteRecords.Entities.Count > 0)
            {
                ...
                salesOrderEntity["gsc_downpayment"] = quote.Contains("gsc_downpayment")
                    ...
                    : new Money(Decimal.Zero);
                salesOrderEntity["gsc_downpaymentdisplay"] = quote.Contains("gsc_downpaymentdisplay")
                   ? quote.GetAttributeValue<Money>("gsc_downpaymentdisplay")
                   : new Money(Decimal.Zero);
                salesOrderEntity["gsc_totaldownpaymentdiscountdisplay"] = quote.Contains("gsc_totaldownpaymentdiscountdisplay")
                   ? quote.GetAttributeValue<Money>("gsc_totaldownpaymentdiscountdisplay")
                   : new Money(Decimal.Zero);
            ...
            else
            {
                ...
                salesOrderEntity["gsc_downpaymentdisplay"] = new Money(0);
                salesOrderEntity["gsc_totaldownpaymentdiscountdisplay"] = new Money(0);
        ...
        public Boolean CheckifOrderHasDownPayment(Entity salesOrderEntity)
        {
            if (salesOrderEntity.Contains("gsc_downpaymentamount"))
            {
                if (salesOrderEntity.GetAttributeValue<Money>("gsc_downpaymentamount").Value != 0)
                    return true;
            }
            return false;
        }

        public Boolean CheckifHasVehicleAllocator(Entity salesOrderEntity)
        {
            Guid branchId = salesOrderEntity.Contains("gsc_branchid") ? salesOrderEntity.GetAttributeValue<EntityReference>("gsc_branchid").Id : Guid.Empty;
            EntityCollection branchCollection = CommonHandler.RetrieveRecordsByOneValue("account", "accountid", branchId, _organizationService, null, OrderType.Ascending,
              new[] { "gsc_vehicleallocator" });
            if (branchCollection != null && branchCollection.Entities.Count > 0)
            {
                Entity branchEntity = branchCollection.Entities[0];
                return branchEntity.Contains("gsc_vehicleallocator");
            }
            return false;
        }

        //Created By: Leslie G. Baliguat, Created On: 05/25/2017
        public Entity UpdatePDIStatus(Entity salesOrder)
        {
            var pdiStatus = salesOrder.Contains("gsc_pdistatus")
                ? salesOrder.GetAttributeValue<OptionSetValue>("gsc_pdistatus").Value
                : 0;

            if (salesOrder.GetAttributeValue<Boolean>("gsc_isreadyforpdi") && pdiStatus == 100000001)
            {
                Entity orderToUpdate = _organizationService.Retrieve(salesOrder.LogicalName, salesOrder.Id,
                    new ColumnSet("gsc_pdistatus"));
                orderToUpdate["gsc_pdistatus"] = new OptionSetValue(100000000);
                _organizationService.Update(orderToUpdate);
